...
- One landing page per tool (`/non-followers`, `/who-unfollowed-me`, `/fans`, `/fake-followers`, `/inactive-following`, `/engagement-report`, `/bookmarks-export`, `/likes-export`, `/thread-unroll`, `/search-export`, `/list-members-export`, `/blocked-muted-export`), generated by `npm run site` from `scripts/site-content.mjs` and sharing the home page's stylesheet, navigation and footer. Each has its own title, description, five-step guide, FAQ, related tools, copy button and structured data (BreadcrumbList, HowTo, FAQPage). The home page links to them from every tool card and the sitemap lists them.
